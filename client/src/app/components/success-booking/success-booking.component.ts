...
import { SELETE_DEFAULT_ROOM_S } from "../../store/actions/room-select.action";
@Component({
  ...
  async getShowUserQueryParams(queryParams) {
    try {
      if (queryParams.length === 24) {
        let bookingList = await this._booking
          .getBookingListOneData(queryParams)
          .toPromise();
        ...
        this.loadingShow = true;
        this.msg_err_show =
          "ไม่สามารถโหลดข้อมูลของ รหัสนี้ได้ id bookingList มีจำนวนไม่เป็นจริง ";
      }
    } catch (error) {
      this.loadingShow = true;
