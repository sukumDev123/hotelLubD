...
export class BookingComponent implements OnInit {
  ...
  check_in: any = this.date_now_variable;
  check_out: any = this.date_now_variable;
  ...
  async ngOnInit() {
    ...
    try {
      ...
      this._msg.set_msg_type(
        error.error.message,
        "Status is not success loading data array. ",
        "err",
        new Date().getHours(),
        true,
        error.status
  ...
  storeRoomSelect() {
    this._store.select<any>("room_select").subscribe(
      suc => {
  ...
  getIndexOfDataNotEixstsAndExists(
    rooms_temp_main: RoomDetail[],
    select: number
  ): Promise<any> {
    // sub method of roomIsEmptyCheck
    return new Promise(res => {
      ...
      rooms_temp_main.forEach((room, i) => {
        ...
        room.liveLatest.forEach((dateLatest, j) => {
          const dateIn = new Date(room.liveDate[j]).valueOf();
          const dateLatestLet = new Date(dateLatest).valueOf();
  ...
  indexEndSelect(indexOfRooms: any, roomsNum: number): Promise<any> {
    // sub method of roomIsEmptyCheck
    return new Promise(async res => {
      let { e_index, e_not_index } = indexOfRooms;
      ...
      if (e_index.length === roomsNum) {
        ...
        let data_is_not_exists = await this.dataIsNotExistsSelect(
          e_index,
          e_not_index
        );
  ...
  roomIsEmptyCheck(date_select: Date, rooms: RoomDetail[]): Promise<any> {
    return new Promise(async (res, rej) => {
      ...
      if (
        ...
        selectToString === dateNotToString
      ) {
        ...
        const index_end_selectTed = await this.indexEndSelect(
          indexOfRooms,
          rooms.length
        );
  ...
  async cal_num_night(dateInput, date_is, caseIsThis = "yes") {
    try {
      ...
      if (date_is === "date_in") {
        ...
        const room_temps = await this.roomIsEmptyCheck(
          dateInput,
      ...
      this.cal_price_num.night_num = this.days_between(
  ...
  cal_price_num_function(data: any = []): CalPriceNum {
    ...
    let price_ = data.reduce(
      (sum, room_input) => (sum += room_input.priceRoom),
      0
    );
    let nigh_ = this.cal_price_num.night_num ? this.cal_price_num.night_num : 1;
    let total_ =
      num_ || price_ || nigh_
        ? this.cal_price_total_everything(price_, nigh_)
        : 0;
    cal_ = {
  ...
  bookingFunction() {
    ...
    if (
      this.cal_price_num.total_price_room &&
      this.cal_price_num.night_num > 0
    ) {
      if (this.check_this_is_user(this.booking_now.user_booking)) {
        if (this.booking_now.room.length) {
          ...
          this._booking.bookingNowService(this.booking_now).subscribe(
            suc => {
              this.booking_now = this.data_is_defult();
              this._msg.set_msg_type(
                suc.message,
  ...
  ok_user_input() {
    if (this.check_this_is_user(this.booking_now.user_booking)) {
      ...
      this._msg.set_msg_type(
        "โปรดกรอกข้อมูล ของลูกค้าให้ครบ",
        "input is empty feild",
        "err",
        new Date().getHours(),
        true,
        123
